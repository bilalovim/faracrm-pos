...
class Contact(AuditMixin, DotModel):
    ...
    async def find_operator_by_value(
        ...
        Используется синхронизацией номеров телефонии: extension (sip) или
        обычный номер (phone) сотрудника → сам сотрудник (user_id). Тип контакта
        обычно НЕ задают — важно совпадение значения, а не графа, в которой оно
        лежит (у Sipuni/МегаФона sip-контакта может не быть вовсе).
        """
        if not value:
            return None
